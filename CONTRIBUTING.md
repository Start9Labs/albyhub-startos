...
This repo packages [Alby Hub](https://github.com/getAlby/hub) for StartOS.
...
Alby Hub runs the `ghcr.io/getalby/hub` image. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/getalby/hub:v<new version>`.
