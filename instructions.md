...
- A one-time choice of Lightning backend: the LND, Core Lightning, or phoenixd service running on this StartOS, or Alby Hub's bundled LDK light node.
...
1. If you intend to use **LND**, **Core Lightning**, or **phoenixd on this server**, install and start that package first. (If you pick **LDK embedded node** instead, skip this step.)
...
- Only the **LND**, **Core Lightning**, **phoenixd**, and **LDK** backends are available; other upstream options (Cashu, …) are not offered.
