...
**Key difference:** On StartOS, you must complete a mandatory setup task to choose your Lightning backend (LND, Core Lightning, or LDK) before Alby Hub can start. This choice is permanent.
...
|---------|-----------------|----------------|
| `LN_BACKEND_TYPE` | Env var (LND, LDK, CLN, Phoenixd, Cashu, …) | One-time action (LND, CLN, or LDK only) |
...
| `ENABLE_ADVANCED_SETUP` | Env var (default: unset) | Set to `false` when using LND or CLN |
...
**Note:** Upstream supports several backends (LDK, LND, CLN, Phoenixd, Cashu, …). StartOS exposes LND, Core Lightning, and LDK.
...
1. **Subset of upstream backends supported** — StartOS offers LND, Core Lightning, and LDK only; Phoenixd, Cashu, and other upstream backends are not available
2. **Backend selection is permanent** — cannot switch between LND, Core Lightning, and LDK without reinstalling
3. **Advanced setup disabled for LND/CLN** — `ENABLE_ADVANCED_SETUP=false` is set, preventing backend changes via web UI
4. **No PostgreSQL support** — only embedded SQLite database
5. **No custom node connection** — must use the StartOS LND or Core Lightning dependency; cannot connect to external nodes
...
startos_managed_env_vars:
  ...
  - ENABLE_ADVANCED_SETUP (when LND or CLN)
...
backup_volumes:
  - main
  - startos
backend_options: [LND, CLN, LDK]  # upstream supports more: LDK, LND, CLN, Phoenixd, Cashu, …
```
